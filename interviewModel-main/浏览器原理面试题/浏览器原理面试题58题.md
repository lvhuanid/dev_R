...
首先v8引擎会去处理js脚本，node处理node的一些api，而libuv库就是负责调度这些任务到不同的线程中执行，形成一个eventloop，最后以异步的形式去返回结果给v8引擎
...
- Paint：根据 layout 计算出的逻辑布局以及 render tree 的样式信息，计算各个节点的位图效果；这里为了提升渲染效率，会选择将渲染任务拆解成多个，形成 render Layer分层节点
...
浏览器限制并发连接数是为了有利于网络资源的利用和用户体验。下面是一些原因：
